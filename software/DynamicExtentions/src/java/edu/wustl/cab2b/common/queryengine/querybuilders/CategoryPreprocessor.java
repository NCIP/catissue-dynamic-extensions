...
package edu.wustl.cab2b.common.queryengine.querybuilders;
...
public class CategoryPreprocessor
{
	...
	private static final Logger logger = edu.wustl.common.util.logger.Logger
			.getLogger(CategoryPreprocessor.class);
	...
	public CategoryPreprocessorResult processCategories(IQuery query)
	{
		clear();
		this.query = query;
		categoryPreprocessorResult = new CategoryPreprocessorResult();
		processCategories();
	...
	{
		for (IExpression expression : catExprs)
		{
	...
	private void processCategoryExpression(IExpression expr)
	{
		...
		else
		{
			...
			{
				if (catExprs.contains(parentExpr))
				{
				...
				IExpression transformedExpr = transformCategoryExpression(expr, association
						.getTargetEntity());
				...
				catch (CyclicException e)
				{
					...
					throw new RuntimeException("Problem in code... might be due to invalid input.",
							e);
				}
	...
	private Category pivot(Category category, EntityInterface requiredRoot)
	{
		...
		List<CategorialClass> catClassesPath = findCategorialClassesPathFromRoot(clonedCat
				.getRootClass(), requiredRoot);
	...
	private CategorialClass cloneCategorialClass(CategorialClass categorialClassToClone)
	{
		...
		for (CategorialAttribute origAttr : categorialClassToClone
			...
			EntityInterface requiredRoot)
	{
		...
		{
			List<CategorialClass> pathFromChild = findCategorialClassesPathFromRoot(childCatClass,
					requiredRoot);
	...
	private IExpression transformCategoryExpression(IExpression catExpr, EntityInterface entryPoint)
	{
		...
		IExpression rootExpr = createExpression(rootCatClass.getCategorialClassEntity(), catExpr
				.isInView());
		...
		for (int i = 0; i < catExpr.numberOfOperands(); i++)
		{
			...
			if (operand instanceof IExpression)
			{
				...
				for (int j = 1; j < catClassesPath.size(); j++)
				{
					...
					List<IAssociation> associations = catClass.getPathFromParent()
							.getIntermediateAssociations();
					TreeNode<IExpression> newExprNode = createExpressionsForAssociations(lastExpr,
							associations, new HashSet<IExpression>(), catExpr.isInView(),
							lastExprNode);
				...
				while (!currCatClasses.isEmpty())
				{
					...
					for (CategorialClass categorialClass : currCatClasses)
					{
						...
						TreeNode<IExpression> newExprNode = createExpressionsForAssociations(
								exprForCatClass.get(parentCatClass), categorialClass
										.getPathFromParent().getIntermediateAssociations(),
								possiblyRedundantExprs, catExpr.isInView(), treeNodeForCatClass
										.get(parentCatClass));
			...
			if (numOperandsAdded > 1)
			{
				...
				int operandNesting = precedingConnNesting > followingConnNesting
						? precedingConnNesting
						: followingConnNesting;
				// add operandNesting + 1 parantheses.
				for (int j = 0; j <= operandNesting; j++)
				{
					rootExpr.addParantheses(initialRootExprSize, initialRootExprSize
							+ numOperandsAdded - 1);
				}
			}
		}
		...
		{
			IConnector<LogicalOperator> connector = catExpr.getConnector(entry.getKey(), entry
					.getKey() + 1);
			...
			Set<IExpression> possiblyRedundantExprs)
	{
		...
		for (IExpression possiblyRedundantChildExprId : getJoinGraph().getChildrenList(
	...
	private TreeNode<IExpression> createExpressionsForAssociations(IExpression parentExpr,
			List<IAssociation> associationsToLastChild, Set<IExpression> expressionsAdded,
			boolean inView, TreeNode<IExpression> parentExprNode)
	{
	...
	protected AttributeInterface findSourceAttribute(AttributeInterface catAttr,
			CategorialClass categorialClass)
	{
		return categorialClass.findSourceAttribute(catAttr);
	}
	...
	private void addOperandToExpr(IExpression parentExpr, IExpressionOperand operand)
	{
		...
		{
			parentExpr.addOperand(QueryObjectFactory.createLogicalConnector(LogicalOperator.And),
					operand);
		}
		else
		{
