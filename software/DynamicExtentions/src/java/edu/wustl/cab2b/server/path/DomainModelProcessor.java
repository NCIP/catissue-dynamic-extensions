...
public class DomainModelProcessor
{
	...
	private static final Logger logger = edu.wustl.common.util.logger.Logger
			.getLogger(DomainModelProcessor.class);
			...
			throws DynamicExtensionsSystemException, DynamicExtensionsApplicationException
	{
		...
		entityGroup.setName(Utility.createModelName(domainModel.getProjectLongName(), domainModel
				.getProjectVersion()));
	...
	protected EntityInterface createEntity(UMLClass umlClass)
	{
		...
		for (UMLAttribute umlAttribute : attributes)
		{
			...
			if (attribute != null)
			{ // to bypass attributes of invalid data-types
	 ...
	 * @throws DynamicExtensionsSystemException
	 */
	protected void addAssociation(UMLAssociation umlAssociation,
			Map<String, List<String>> parentIdVsChildrenIds)
			throws DynamicExtensionsSystemException
	{
		UMLAssociationEdge srcEdge = umlAssociation.getSourceUMLAssociationEdge()
		...
		UMLAssociationEdge tgtEdge = umlAssociation.getTargetUMLAssociationEdge()
				.getUMLAssociationEdge();
		...
		for (int i = 0; i < childrenOfSrc.size(); i++)
		{
			for (int j = 0; j < childrenOfTgt.size(); j++)
			{
				...
				RoleInterface srcRole = DynamicExtensionUtility.cloneRole(association
						.getSourceRole());
				RoleInterface tgtRole = DynamicExtensionUtility.cloneRole(association
						.getTargetRole());
	...
	private RoleInterface getRole(UMLAssociationEdge edge)
	{
		...
		//Bug# 10295: Work around to fix the max cardinality = 0. Assuming these cases are MANY
		if (maxCardinality == 0)
		{
	 ...
	 *            value as list of UML-id of all children classes.
	 */
	protected void processInheritance(Map<String, List<String>> parentIdVsChildrenIds)
	{
	 ...
	 */
	protected void markInheritedAttributes(EntityGroupInterface entityGroup)
	{
		for (EntityInterface entity : entityGroup.getEntityCollection())
		{
	 ...
	 */
	protected List<EntityInterface> getAllChildren(String umlIdOfParent,
			Map<String, List<String>> parentIdVsChildrenIds)
	{
	 ...
	 */
	protected AssociationInterface getAssociation(EntityInterface sourceEntity)
	{
	 ...
	 */
	protected void setEntityVsIndex(Map<EntityInterface, Integer> entityVsIndex)
	{
	...
	protected EntityGroupInterface saveEntityGroup(EntityGroupInterface entityGroup)
			throws DynamicExtensionsSystemException, DynamicExtensionsApplicationException
	{
